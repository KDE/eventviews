...
bool Agenda::eventFilter_mouse(QObject *object, QMouseEvent *me)
{
    ...
    case QEvent::MouseMove: {
        ...
        if (object != this) {
            ...
            KCalendarCore::Incidence::Ptr incidence = moveItem ? moveItem->incidence() : KCalendarCore::Incidence::Ptr();
...
Agenda::MouseActionType Agenda::isInResizeArea(bool horizontal, QPoint pos, const AgendaItem::QPtr &item)
{
    ...
    QPoint contpos = gridToContents(gridpos + QPoint((QApplication::isRightToLeft()) ? 1 : 0, 0));
...
void Agenda::placeAgendaItem(const AgendaItem::QPtr &item, double subCellWidth)
{
    ...
    // right lower corner
    QPoint pt1 = gridToContents(QPoint(item->cellXLeft() + item->cellWidth(), item->cellYBottom() + 1));
